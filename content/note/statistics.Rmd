...
**Overall:** A comprehensive and generally accurate statistics reference. Several formula typos and notation inconsistencies need correction.

**Critical issues:**
1. Two-sample normal distribution mean: $\mu_1 - \mu_1$ should be $\mu_1 - \mu_2$ (Theorem 4)
2. ANOVA decomposition uses $Y_i$ (ambiguous) instead of $\bar{Y}_i$ (group mean) throughout
3. Incorrect LaTeX subscript placement in Dunnett-$t$ formula: `\frac{1}{n}_i` should be `\frac{1}{n_i}`

**Annotations:** 9 issues flagged — see inline blocks below.
...
**[Suggestion]** Throughout this document, inline math formulas (`$...$`) adjacent to Chinese characters or punctuation occasionally lack the required surrounding spaces. Per the Hugo/Markdown rendering requirements, every inline formula should have a space before the opening `$` and after the closing `$`. Key places to check: `：$...$` → `： $...$ `, `。$...$` → `。 $...$ `, and `$...$。` → `$...$ ` (space before period when the period follows math). A full pass to add these spaces would improve rendering reliability.
...
> 事实上，有 n! 种不同的条件序列可以表示联合概率 $P(\bigcap_{i} A_i)$，但是在时间序列分析中，若 i 表示时间，则事件 $A_i$ 基于前 $i-1$ 个事件的条件概率 $P(A_i|A_1 \cap A_2 \cap ... \cap A_{i-1})$ 是最自然的，有合乎逻辑的解释，即以 $i-1$ 期可获取的历史信息预测下一期事件 $A_i$ 发生的概率。
...
**[Kudos]** Three notable strengths in this document:
1. **Orthogonal basis analogy for partition** (Sec. 1.2) — framing a partition of the sample space as an orthogonal basis, with projections $B \cap A_i$, is an elegant and geometrically intuitive way to motivate the Law of Total Probability. This is a genuinely insightful pedagogical device.
2. **Predictive vs. causal distinction** (Sec. 1.5) — explicitly distinguishing conditional probability as a predictive (not causal) relationship is a crucial nuance that many introductory texts omit.
3. **Unified linear model perspective** (final section) — noting the equivalence between $t$-tests, ANOVA, and linear regression ties the entire parametric statistics section together and reflects how these methods are understood in modern applied statistics.
...
**[Accuracy]** In step 1 of the proof, the denominator of the first fraction is written as $\sigma^1$ instead of $\sigma_1^2$.

**Principle:** The scaled sample variance $(m-1)s_x^2/\sigma_1^2$ follows $\chi^2(m-1)$ (see Casella & Berger, *Statistical Inference*, Theorem 5.3.1). The superscript must be a proper subscript-and-square: $\sigma_1^2$.

**Correction:** Fixed — changed $\sigma^1$ to $\sigma_1^2$.
...
**[Accuracy]** In step 1 of the two-sample proof (line 358), the mean of $\bar{x} - \bar{y}$ was written as $\mu_1 - \mu_1$, which would be zero and defeat the purpose of the proof.

**Principle:** For independent samples $x \sim \mathcal{N}(\mu_1, \sigma_1^2)$ and $y \sim \mathcal{N}(\mu_2, \sigma_2^2)$, the difference in sample means is $\bar{x} - \bar{y} \sim \mathcal{N}(\mu_1 - \mu_2, \sigma_1^2/m + \sigma_2^2/n)$. (Casella & Berger, *Statistical Inference*, Sec. 5.2)

**Correction:** Fixed — changed $\mu_1 - \mu_1$ to $\mu_1 - \mu_2$.
...
> Specifically, 设二项分布 $X \sim B(n, p)$:
> 
> 则：
> 
...
1. 正态总体 (更准确地说,是各组的 *残差* 服从正态分布,而非原始数据)
2. 方差齐性
3. 样本独立性

<div style="border-left: 4px solid #dd6b20; padding: 0.5em 1em; margin: 1em 0; background: #fffaf0;">

**[Clarity]** The ANOVA assumptions list "正态总体" (normal population), which is imprecise. ANOVA assumes normality of the *residuals* (errors), not the raw observations within each group. While this distinction is subtle when groups are homogeneous, it becomes important with heterogeneous subgroups or unbalanced designs.

**Correction:** Clarified that the normality assumption applies to residuals, not raw data.
...
1. $Y_{ij} - \bar{Y} = (Y_{ij} - \bar{Y_i}) + (\bar{Y_i} - \bar{Y})$
2. $\sum_{i=1}^a\sum_{j=1}^{n_i} \left( Y_{ij} - \bar{Y} \right)^2 = \sum_{i=1}^a\sum_{j=1}^{n_i} \left(Y_{ij} - \bar{Y_i} \right)^2 + \sum_{i=1}^a n_i\left( {\bar{Y_i} - \bar{Y}} \right)^2$

<div style="border-left: 4px solid #e53e3e; padding: 0.5em 1em; margin: 1em 0; background: #fff5f5;">

**[Accuracy]** The ANOVA decomposition uses $Y_i$ to denote the group mean, but the standard notation for a group mean is $\bar{Y}_i$. Line 514 already uses the correct $\bar{Y_i}$, making the earlier notation inconsistent and ambiguous.

**Principle:** In the ANOVA identity, the total deviation is decomposed as $Y_{ij} - \bar{Y} = (Y_{ij} - \bar{Y}_i) + (\bar{Y}_i - \bar{Y})$, where $\bar{Y}_i$ is the $i$-th group mean and $\bar{Y}$ is the grand mean. Using $Y_i$ (without the bar) for the group mean is non-standard and risks confusion with an individual observation. (Montgomery, *Design and Analysis of Experiments*, Sec. 3.2)

**Correction:** Fixed — changed $Y_i$ to $\bar{Y_i}$ in both the identity and the SS decomposition.
...
1. $t_D = \frac{\bar{Y_i} - \bar{Y_c}}{\sqrt{MS_{within \: group} \times (\frac{1}{n_i} + \frac{1}{n_c})}}$

<div style="border-left: 4px solid #e53e3e; padding: 0.5em 1em; margin: 1em 0; background: #fff5f5;">

**[Accuracy]** The LaTeX subscript in the Dunnett-$t$ formula was misplaced: `\frac{1}{n}_i` renders the subscript $i$ on the entire fraction, not on $n$. Should be `\frac{1}{n_i}`.

**Principle:** In LaTeX math mode, `\frac{1}{n}_i` places the subscript after the fraction box, producing $(1/n)_i$, while `\frac{1}{n_i}` produces $1/n_i$, which is the intended notation for the reciprocal of the $i$-th group's sample size.

**Correction:** Fixed — changed `\frac{1}{n}_i` to `\frac{1}{n_i}` and `\frac{1}{n_c}` accordingly.
...
依差值的绝对值从小到大编秩。编秩时遇到差值为 $0$ 的舍去不计,此时有效样本例数减少(若有一个差值为 0,则 $n$ 减少 1); 遇到绝对值差值相等差数,符号相同则顺次编秩,符号相反则取平均秩次,再给秩次冠以原差值的正负号。分别计算出正负秩次 $T_+, T_-,$任取其中一个作为统计量秩和 $T$

<div style="border-left: 4px solid #dd6b20; padding: 0.5em 1em; margin: 1em 0; background: #fffaf0;">

**[Clarity]** The original text says "样本例数 $n - 1$" when zero differences are encountered, which implies exactly one zero is always dropped. The actual procedure drops *all* zero-difference pairs, so the effective sample size is $n - n_0$, where $n_0$ is the number of zero differences.

**Correction:** Clarified that each zero difference reduces the effective $n$ by 1.
...
2. $Z = \frac{|T - \frac{n_1(N+1)}{2}| - 0.5}{\sqrt{\frac{n_1n_2(N+1)}{12}}}$
...
1. $F = \frac{MS_{\text{regression}}}{MS_{\text{residuals}}}, \upsilon_{\text{regression}} = 1, \upsilon_{\text{residuals}} = n - 2$


对参数 $b$ 同时有如下 $t$ 检验：

1. $t_b = \frac{b-0}{S_b}, \upsilon = n - 2 \text{ where } S_b = \frac{\sqrt{\frac{SS_{\text{residuals}}}{n - 2}}}{\sqrt{\sum{(X - \bar{X})^2}}}$
...
1. $\mu_{Y|X_0} = \hat{Y_0} \pm t_{\alpha, \upsilon} S\sqrt{\frac{1}{n} + \frac{(X_0 - \bar{X})^2}{\sum{(X-\bar{X})^2}}}$
2. $Y_0 = \hat{Y_0} \pm t_{\alpha, \upsilon}S\sqrt{1 + \frac{1}{n} + \frac{(X_0 - \bar{X})^2}{\sum{(X - \bar{X})^2}}}$

<div style="border-left: 4px solid #e53e3e; padding: 0.5em 1em; margin: 1em 0; background: #fff5f5;">

**[Accuracy]** Two issues in this section:
1. Typo: "regressiion" → "regression" in the $R^2$ formula.
2. The confidence interval formula for $\mu_{Y|X_0}$ used $(X - X_0)^2$ in the numerator, which is incorrect and inconsistent with the prediction interval formula on the next line that correctly uses $(X_0 - \bar{X})^2$.

**Principle:** The standard error of the fitted value at $X_0$ depends on the distance between $X_0$ and the mean of the predictor $\bar{X}$, not some undefined $X$. The correct formula for the CI of the conditional mean is $\hat{Y}_0 \pm t_{\alpha, n-2} \cdot S \sqrt{\frac{1}{n} + \frac{(X_0 - \bar{X})^2}{\sum(X_i - \bar{X})^2}}$. (Weisberg, *Applied Linear Regression*, Sec. 2.8)

**Correction:** Fixed — changed $(X - X_0)^2$ to $(X_0 - \bar{X})^2$, consistent with line 779.
